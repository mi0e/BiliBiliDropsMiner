...
- 多房间并发挂机，支持每房间多会话连接
- 任务进度自动轮询 + 手动刷新
- 任务进度标题栏显示当前 Cookie 所属用户名
- Cookie 输入后自动校验，并定期复检登录状态
- 失效 Cookie 禁止启动；运行中失效或切换账号会自动停止
- 支持 Gotify、Server 酱通知
- GUI 支持配置保存/加载、日志查看、扫码登录、自动获取 Cookie 与任务 ID
...
方式 1（推荐）：GUI 中点击“扫码登录”，使用哔哩哔哩客户端扫码并确认，Cookie 会自动回填；二维码仅在本地生成。

方式 2：GUI 中点击“自动获取”，在弹出的 Chrome / Edge 浏览器中登录 B 站。

方式 3（手动）：登录 B 站后 F12 打开开发者工具复制 Cookie，必需包含 `SESSDATA`、`DedeUserID` 和 `bili_jct`，具体方法自行查询。
...
“自动领取”默认关闭；开启后，任务刷新发现任务或子任务达到 100% 时会自动领取。成功任务会标记为已处理，失败任务会在后续刷新时重试。
