...
GUI 中点击 Cookie 输入框旁的「自动获取」→ 在弹出的浏览器中登录 B 站 → 登录成功后 Cookie 自动填入（含 httpOnly 字段），浏览器自动关闭。
...
> 注意：手动提取无法获取 httpOnly 字段，推荐使用自动获取。
...
GUI 中点击任务 ID 输入框旁的「自动获取」→ 在弹出的浏览器中进入任意带有任务的直播间 → 工具自动捕获 task_ids 并填入，浏览器自动关闭。

> 支持在浏览器中新开标签页，任意标签页触发的任务请求都能被捕获。
